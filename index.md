...
{% for key in latestIssue["highlights"] limit:3 %}
{% assign post = site.posts | find_exp:"item", "item.permalink contains key" %}
<span class="nf">{{ site.color-emoji[post.category] }}</span> {{ post.title | markdownify | remove: '<p>' | remove: '</p>' }} {% if post.authors != nil %} | <span class="archive-author">{{ post.authors | join: ", " }}</span> {% endif %} <br>
{% endfor %}
